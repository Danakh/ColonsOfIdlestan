...
describe('Civilization', () => {
  describe('méthodes utilitaires avec IslandMap', () => {
    it('devrait retourner le nombre de routes correct', () => {
      ...
      expect(civilization.getRoadCount(islandMap)).toBe(0);
      ...
      islandMap.addRoad(road1, civId);

      expect(civilization.getRoadCount(islandMap)).toBe(1);

      const road2 = Edge.create(seHex, sHex);
      islandMap.addRoad(road2, civId);

      expect(civilization.getRoadCount(islandMap)).toBe(2);
    });
    ...
    it('devrait retourner le nombre de villes correct', () => {
      ...
      expect(civilization.getCityCount(islandMap)).toBe(1);
      ...
      if (newCityVertex) {
        islandMap.addCity(newCityVertex, civId, CityLevel.Outpost);
        expect(civilization.getCityCount(islandMap)).toBe(2);
      }
    });
    ...
    it('devrait retourner la somme des niveaux de villes correcte', () => {
      ...
      expect(civilization.getTotalCityLevel(islandMap)).toBe(1);
    ...
    it('devrait retourner le nombre de bâtiments correct', () => {
      ...
      // Donc 1 bâtiment
      expect(civilization.getBuildingCount(islandMap)).toBe(1);
    ...
    it('devrait retourner 0 pour toutes les statistiques si la civilisation n\'a rien', () => {
      ...
      islandMap.registerCivilization(newCivId);
      const newCivilization = new Civilization(newCivId);

      expect(newCivilization.getRoadCount(islandMap)).toBe(0);
      expect(newCivilization.getCityCount(islandMap)).toBe(0);
      expect(newCivilization.getTotalCityLevel(islandMap)).toBe(0);
      expect(newCivilization.getBuildingCount(islandMap)).toBe(0);
    });
    ...
    it('devrait compter correctement plusieurs villes avec différents niveaux', () => {
      ...
      if (city1Vertex && city2Vertex) {
        // Ajouter une ville Outpost (niveau 0)
        islandMap.addCity(city1Vertex, civId, CityLevel.Outpost);
        // Ajouter une ville Town (niveau 2)
        islandMap.addCity(city2Vertex, civId, CityLevel.Town);

        // Vérifier les statistiques
        // 1 ville initiale (Colony niveau 1) + 1 Outpost (niveau 0) + 1 Town (niveau 2) = 3 villes
        expect(civilization.getCityCount(islandMap)).toBe(3);
        // Total niveau: 1 + 0 + 2 = 3
        expect(civilization.getTotalCityLevel(islandMap)).toBe(3);
        // Bâtiments: 1 (TownHall de la première ville) + 0 (Outpost) + 1 (TownHall de Town) = 2
        expect(civilization.getBuildingCount(islandMap)).toBe(2);
      }
    });
  });
});
